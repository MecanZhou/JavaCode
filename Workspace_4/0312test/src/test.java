...
class Chat extends FederateAmbassador_Impl
{
      ...
      try {
       ...
       // 
         ...
         while (true) {
            ...
            parameters.add(_parameterIdText1, value);
            parameters.add(_parameterSender1, encodeInt(myInstanceId));

            _rtiAmbassador.sendInteraction(_messageId1, parameters, null);
         }
   ...
   {
      try {
         if (interactionClass == _messageId1) {
            String message1 = "";
            String sender1 = "Unknown";
            for (int i = 0; i < theInteraction.size(); ++i) {
               if (theInteraction.getParameterHandle(i) == _parameterIdText1) {//根据上面订购交互关系取出相应的数据
                  message1 = new String(theInteraction.getValueReference(i));
               }
               if (theInteraction.getParameterHandle(i) == _parameterSender1) {
                  int senderId = decodeInt(theInteraction.getValueReference(i));
                  sender1 = (String)_participants.get(new Integer(senderId));
               }
            }
            System.out.println(sender1 + ": " + message1);
         }
